...
第一次通过 CLI/TUI 创建新 Session 时，如果用户配置不存在，CodeCraft 会按
当前默认值自动创建 `~/.codecraft/config.toml`。创建采用排他模式，并发启动也
只有一个进程能够成功写入；已有文件永远不会被自动覆盖。安装包本身不会修改
用户主目录。生成文件只记录 API Key 的环境变量名，不会记录密钥值。
...
Session 创建时解析后的 `cwd`，既是相对路径的起点，也是内置文件工具、检索、
项目指令和沙箱写入共同使用的唯一 workspace 边界。单次 bash 调用可以选择它
下面的子目录作为工作目录，但不会改变或扩大 Session 边界。
